...
ROSTER_COLUMNS = [
    'Formatted_Name', 'Normalized_Team', 'Team_Candidates', 'IP',
    'SO', 'H', 'R', 'ER', 'BB', 'ERA', 'WHIP', 'Pitch_Count'
]


def normalize_app_mode(mode):
    """Normalize sidebar mode values."""
    ...
    # Priority-ordered matching to avoid accidental partial matches
...
def fetch_pitcher_statcast(pitcher_name, season):
    """Fetch and normalize pitch-level Statcast data for one pitcher and season."""
    if not isinstance(pitcher_name, str) or not pitcher_name.strip():
    ...
    if id_df is None or id_df.empty or 'key_mlbam' not in id_df.columns:
        return pd.DataFrame(), None
    ...
    required_location_cols = ['plate_x', 'plate_z', 'sz_top', 'sz_bot']
    if not all(column in data.columns for column in required_location_cols):
        return pd.DataFrame(), mlbam_id

    data = data.dropna(subset=required_location_cols).copy()
    zone_height = data['sz_top'] - data['sz_bot']
    data = data[zone_height > 0].copy()
    if data.empty:
        return pd.DataFrame(), mlbam_id

    data['z_norm'] = (data['plate_z'] - data['sz_bot']) / (data['sz_top'] - data['sz_bot'])
    description = data['description'].fillna('') if 'description' in data.columns else pd.Series('', index=data.index)
    data['is_taken'] = description.isin(['called_strike', 'ball', 'blocked_ball'])
    data['is_called_strike'] = (description == 'called_strike').astype(int)
    data['is_swing'] = description.isin(['swinging_strike', 'foul', 'foul_tip', 'hit_into_play', 'swinging_strike_blocked'])
    data['is_whiff'] = description.isin(['swinging_strike', 'swinging_strike_blocked']).astype(int)
    data['is_in_zone'] = (
        (data['plate_x'].abs() <= 0.83)
        & (data['plate_z'] >= data['sz_bot'])
        & (data['plate_z'] <= data['sz_top'])
    )

    if 'game_pk' in data.columns and 'at_bat_number' in data.columns:
        data['pa_id'] = data['game_pk'].astype(str) + "_" + data['at_bat_number'].astype(str)
    else:
        data['pa_id'] = data.index.astype(str)
    ...
    cols_to_keep = [
        ...
        'post_home_score', 'post_away_score', 'pfx_x', 'pfx_z', 'launch_speed',
        'launch_angle', 'hit_distance_sc', 'release_extension', 'delta_run_exp'
    ]
    data = data[[column for column in cols_to_keep if column in data.columns]]

    for column in [
        'plate_x', 'plate_z', 'sz_top', 'sz_bot', 'z_norm', 'release_speed',
        'release_spin_rate', 'pfx_x', 'pfx_z', 'launch_speed', 'launch_angle',
        'hit_distance_sc', 'release_extension', 'delta_run_exp'
    ]:
...
# -----------------------------------------------------------------------------
def fetch_mlb_pitching_roster(season):
    """Fetch season pitching summaries directly from MLB StatsAPI."""
    url = (
        "https://statsapi.mlb.com/api/v1/stats?"
        f"stats=season&group=pitching&season={int(season)}&sportIds=1&limit=1000"
    )
    payload = fetch_json(url)
    ...
    for stats_group in payload.get('stats', []) or []:
        for split in stats_group.get('splits', []) or []:
            ...
            rows.append({
                ...
                'Pit': stat.get('numberOfPitches', 0),
...
def get_active_pitchers_list(season, min_pitches=DEFAULT_MIN_PITCHES):
    ...
    if df_p is None or df_p.empty:
        df_p = fetch_mlb_pitching_roster(season)
    ...
    # Normalize every field used by the aggregate, including StatsAPI strings.
    for col in ['IP', 'Pitch_Count', 'SO', 'H', 'R', 'ER', 'BB', 'ERA', 'WHIP']:
...
def build_pitch_velocity_fatigue_plot(df_p):
    """Plot pitch velocity against cumulative pitch count within each outing."""
    required = {'game_pk', 'release_speed'}
    if df_p.empty or not required.issubset(df_p.columns):
        return go.Figure()

    work = df_p[['game_pk', 'release_speed']].copy()
    work['release_speed'] = pd.to_numeric(work['release_speed'], errors='coerce')
    work = work.dropna(subset=['release_speed']).copy()
    if work.empty:
        return go.Figure()

    work['Pitch Count'] = work.groupby('game_pk').cumcount() + 1
    work['Outing'] = work['game_pk'].astype(str)
    fig = px.scatter(
        work,
        x='Pitch Count',
        y='release_speed',
        color='Outing',
        opacity=0.65,
        title='Pitch Velocity and Pitch Count',
        labels={'release_speed': 'Pitch Velocity (MPH)', 'Outing': 'Outing'}
    )
    fig.update_layout(template='plotly_dark', height=380, showlegend=False)
    return fig


def build_exit_velocity_launch_angle_plot(df_p):
    """Plot batted-ball launch angle against exit velocity."""
    required = {'launch_angle', 'launch_speed'}
    if df_p.empty or not required.issubset(df_p.columns):
        return go.Figure()

    work = df_p[['launch_angle', 'launch_speed']].copy()
    ...
    fig = px.scatter(
        ...
        labels={'launch_angle': 'Launch Angle (degrees)', 'launch_speed': 'Exit Velocity (MPH)'},
        trendline=None
    )
    fig.update_layout(template='plotly_dark', height=380)
    return fig
...
def build_velocity_range_by_pitch_type(df_p):
    """Show velocity consistency and outliers for each pitch type."""
    required = {'pitch_name', 'release_speed'}
    if df_p.empty or not required.issubset(df_p.columns):
        return go.Figure()

    work = df_p[['pitch_name', 'release_speed']].copy()
    work['release_speed'] = pd.to_numeric(work['release_speed'], errors='coerce')
    work = work.dropna(subset=['pitch_name', 'release_speed']).copy()
    if work.empty:
        return go.Figure()
    ...
    fig = px.box(
        ...
        y='release_speed',
        points='outliers',
        title='Velocity Range by Pitch Type',
        labels={'pitch_name': 'Pitch Type', 'release_speed': 'Pitch Velocity (MPH)'}
    )
    fig.update_layout(template='plotly_dark', height=380)
    return fig


def build_contact_quality_distribution(df_p):
    """Show the distribution of exit velocity allowed on batted balls."""
    if df_p.empty or 'launch_speed' not in df_p.columns:
        return go.Figure()

    work = pd.to_numeric(df_p['launch_speed'], errors='coerce').dropna().to_frame('launch_speed')
    if work.empty:
        return go.Figure()
    ...
    fig = px.histogram(
        ...
        labels={'launch_speed': 'Exit Velocity (MPH)', 'count': 'Batted Ball Count'}
    )
    fig.update_layout(template='plotly_dark', height=380)
    return fig


def build_projected_batted_ball_distance(df_p):
    """Plot launch angle against Statcast's measured batted-ball distance."""
    required = {'launch_angle', 'hit_distance_sc'}
    if df_p.empty or not required.issubset(df_p.columns):
        return go.Figure()

    work = df_p[['launch_angle', 'hit_distance_sc']].copy()
    work['launch_angle'] = pd.to_numeric(work['launch_angle'], errors='coerce')
    ...
    fig = px.scatter(
        ...
        x='launch_angle',
        y='hit_distance_sc',
        title='Projected Batted Ball Distance',
        labels={'launch_angle': 'Launch Angle (degrees)', 'hit_distance_sc': 'Projected Distance (feet)'}
    )
    fig.update_layout(template='plotly_dark', height=380)
    return fig
...
def build_workload_tracker(df_p, lookback=8):
    ...
    if 'inning' not in work.columns:
        work['inning'] = 1
    work['inning'] = pd.to_numeric(work['inning'], errors='coerce').fillna(0)

    game_agg = work.groupby(['game_pk', 'game_date'], as_index=False).agg(
        Pitches=('pitch_name', 'size'),
        HighStressInnings=('inning', lambda s: int(s[s >= 7].nunique()))
    )
...
if app_mode == "📊 Player Analysis":
    ...
    if pitcher_list:
        primary_pitcher = st.sidebar.selectbox(
            "Select Player:",
            pitcher_list,
            index=0,
            key="primary_pitcher"
        ...
        primary_pitcher = None
        st.sidebar.warning("No pitchers are available for this season and pitch filter.")
        st.info("No pitchers are available for Player Analysis. Lower the minimum-pitch filter or select another season.")
        st.stop()

if app_mode == "🔬 Advanced Scouting":
    st.sidebar.subheader("Scouting Selection")
    if pitcher_list:
        scout_pitcher = st.sidebar.selectbox("Select Pitcher:", pitcher_list, index=0, key="scouting_pitcher")
    else:
        scout_pitcher = None
        st.sidebar.warning("No pitchers match the selected season and pitch filter.")
...
if app_mode == "⚡ Live Games":
    ...
    if live_games_df.empty:
        ...
        # Build user-friendly game labels
        live_games_df["label"] = live_games_df.apply(
            lambda row: f"{row['away_team']} @ {row['home_team']} — {row['home_score']}-{row['away_score']} ({row['status']})",
            axis=1
        )

        # Dropdown showing matchup instead of game_pk
        selected_label = st.selectbox("Select a live game", live_games_df["label"])

        # Convert label -> game_pk safely
        selected_game_pk = int(live_games_df.loc[live_games_df["label"] == selected_label, "game_pk"].iloc[0])
    ...
    with st.expander("🧭 Product Roadmap & Live Feature Preview", expanded=True):
        ...
        with c2:
            ...
            required_roster_columns = {'Pitch_Count', 'SO', 'Formatted_Name', 'Normalized_Team', 'IP', 'ERA'}
            if pitchers_df.empty or not required_roster_columns.issubset(pitchers_df.columns):
                st.info("Season roster data is not available for this filter.")
            else:
        ...
        if pitchers_df.empty or not required_roster_columns.issubset(pitchers_df.columns):
            st.info("Recent starts are unavailable because no roster data was returned.")
        else:
...
# -----------------------------------------------------------------------------
elif app_mode == "📊 Player Analysis" and pitcher_list:
    ...
    tab_arsenal, tab_recent, tab_context, tab_zone, tab_trends, tab_batted_ball, tab_workload, tab_matchup = st.tabs(["🎯 Arsenal & Movement", "🗓️ Recent Starts", "🧭 Contextual Filters", "🧠 Strike Zone Grid", "📈 Trends & Whiffs", "💥 Batted Ball & Fatigue", "💪 Workload & Fatigue", "⚔️ Matchup Simulator"])

    with tab_arsenal:
        st.caption("What you are looking at: pitch selection, usage, velocity, spin, and movement. Use this section to understand the pitcher's arsenal and how each pitch behaves.")
    ...
    with tab_recent:
        st.caption("What you are looking at: the pitcher's recent game-by-game workload and called-pitch locations. Use the start selector to inspect how command changed in a specific outing.")
    ...
    with tab_context:
        st.caption("What you are looking at: performance split by raw totals, home/away context, or park adjustment. Use this view to separate pitcher skill from game environment.")
    ...
    with tab_zone:
        st.caption("What you are looking at: where pitches cross the plate, grouped into Heart, Shadow, Chase, and Waste zones. Use the count filter to see how approach changes by count.")
    ...
    with tab_trends:
        st.caption("What you are looking at: velocity and spin trends over time, followed by whiff rate by pitch type. Use this to spot changes in stuff or pitch effectiveness.")
    ...
    with tab_batted_ball:
        st.caption("What you are looking at: fatigue, pitch velocity, contact quality, launch angle, and batted-ball distance. Use these charts to identify velocity loss and the quality of contact allowed.")
        st.subheader('Pitch Fatigue')
        fatigue_fig = build_pitch_velocity_fatigue_plot(primary_data)
        if fatigue_fig.data:
            st.plotly_chart(fatigue_fig, use_container_width=True)
        else:
            st.info('Pitch velocity and pitch-count data are not available for this pitcher.')

        st.subheader('Batted-Ball Quality')
        exit_angle_fig = build_exit_velocity_launch_angle_plot(primary_data)
        if exit_angle_fig.data:
            st.plotly_chart(exit_angle_fig, use_container_width=True)
        else:
            st.info('Exit velocity and launch-angle data are not available for this pitcher.')

        velocity_range_fig = build_velocity_range_by_pitch_type(primary_data)
        if velocity_range_fig.data:
            st.plotly_chart(velocity_range_fig, use_container_width=True)
        else:
            st.info('Pitch-type velocity data are not available for this pitcher.')

        contact_fig = build_contact_quality_distribution(primary_data)
        if contact_fig.data:
            st.plotly_chart(contact_fig, use_container_width=True)
        else:
            st.info('Exit-velocity data are not available for contact-quality analysis.')

        distance_fig = build_projected_batted_ball_distance(primary_data)
        if distance_fig.data:
            st.plotly_chart(distance_fig, use_container_width=True)
        else:
            st.info('Launch-angle and batted-ball distance data are not available for this pitcher.')

    with tab_workload:
        st.caption("What you are looking at: recent pitch volume, high-stress innings, and rolling workload. Use this as a workload-monitoring signal, not a medical diagnosis.")
    ...
    with tab_matchup:
        st.caption("What you are looking at: pitch selection and whiff performance against an individual hitter when batter identifiers are available.")
        matchup_candidates = []
...
# -----------------------------------------------------------------------------
    ...
    st.info("How to use this page: choose a pitcher, select the scouting question you want to answer, then use the filters below the description. Each module explains what its chart measures and where to look for useful signals.")
    ...
    if not pitcher_list:
        ...
        if scout_data.empty:
            ...
            scouting_option = st.selectbox(
                "Select a scouting module",
                [
                    "Statcast Comparison Matrix",
                    "Pitcher vs Team / Division Breakdown",
                    "Hot & Cold Zone Grid Dashboard",
                    "Hitter Vulnerability & Weakness Heatmap",
                    "Fatigue & Workload Risk Score"
                ]
            )
            st.caption("Module selector: choose the exact scouting question you want to investigate.")
            enlarge_chart = st.checkbox(
                "Enlarge selected chart",
                key="advanced_scouting_enlarge_chart",
                help="Switch the selected visualization to a larger display."
            )
            st.caption("Display control: turn this on when you want the selected chart to be easier to inspect.")
            chart_height = 760 if enlarge_chart else 420

            if scouting_option == "Statcast Comparison Matrix":
                st.caption("What you are looking at: compare pitchers on two metrics at a time. Look for the upper-right corner when both metrics are desirable, and inspect outliers or team clusters.")
                metric_options = ['K%', 'Whiff Rate %', 'Called Strike %', 'Zone %', 'Avg Fastball Velo']
                metric_labels = {
                    'K%': 'K%: strikeouts per batter faced',
                    'Whiff Rate %': 'Whiff Rate %: swings missed',
                    'Called Strike %': 'Called Strike %: called strikes per pitch',
                    'Zone %': 'Zone %: pitches in the strike zone',
                    'Avg Fastball Velo': 'Avg Fastball Velo: average fastball speed'
                }
                metric_x, metric_y = st.columns(2)
                with metric_x:
                    x_metric = st.selectbox(
                        ...
                        key="advanced_matrix_x_metric"
                    )
                y_options = [metric for metric in metric_options if metric != x_metric]
                with metric_y:
                    y_metric = st.selectbox(
                        ...
                        index=0,
                        format_func=lambda metric: metric_labels[metric],
                        key="advanced_matrix_y_metric"
                    )
                st.caption(f"Selected comparison: {x_metric} horizontally versus {y_metric} vertically.")
                comparison_df = build_statcast_comparison_matrix(pitcher_list, selected_season, limit=24)
                if not comparison_df.empty and {x_metric, y_metric}.issubset(comparison_df.columns):
                    ...
                    st.info("Comparison data is not available for the selected pitcher pool yet.")

            elif scouting_option == "Pitcher vs Team / Division Breakdown":
                st.caption("What you are looking at: the selected pitcher's pitch volume and whiff rate against each opponent in the available Statcast sample.")
                st.caption("Look for opponents where pitch volume is high and whiff rate is unusually low or high compared with the rest of the sample.")
                breakdown_df = build_opponent_breakdown(scout_data, scout_team)
                st.dataframe(breakdown_df, hide_index=True, use_container_width=True)

            elif scouting_option == "Hot & Cold Zone Grid Dashboard":
                st.caption("What you are looking at: pitch locations and whiff rates by zone. Use the pitch and zone filters to isolate where the pitcher is most or least effective.")
                ...
                st.caption("Pitch filter: choose one pitch to isolate its location pattern. Zone focus: choose a region to inspect its effectiveness.")
                ...
                st.plotly_chart(zone_fig, use_container_width=True, height=chart_height)
                st.dataframe(zone_summary, hide_index=True, use_container_width=True)

            elif scouting_option == "Hitter Vulnerability & Weakness Heatmap":
                st.caption("What you are looking at: the selected hitter's pitch locations and whiff rates by zone. Use this to identify locations that may be vulnerable or difficult to attack.")
                ...
                if hitter_candidates:
                    ...
                    st.caption("Look for zones where this hitter sees many pitches but produces fewer swings and misses, or where the pitcher avoids damage.")
                    weakness_df, weakness_fig = build_hitter_weakness_heatmap(scout_data, selected_hitter)
                    st.plotly_chart(weakness_fig, use_container_width=True, height=chart_height)
            ...
            elif scouting_option == "Fatigue & Workload Risk Score":
                st.caption("What you are looking at: recent pitch volume and a simple workload risk index. Use this to flag rising workload patterns for further review.")
                workload_df, workload_fig = build_workload_risk(scout_data)
                if not workload_df.empty:
                    st.caption("Look for rising pitch totals, repeated high-stress innings, and a Risk Score moving from Normal toward Elevated or High.")
                    ...
                    st.plotly_chart(workload_fig, use_container_width=True, height=chart_height)
...
# -----------------------------------------------------------------------------
# =============================================================================
# DROP-IN RENDER FUNCTIONS FOR PLAYER COMPARISON
# =============================================================================

def render_velocity_spin_comparison(p1, p2):
    st.subheader("Velocity & Spin Comparison")
    df = pd.DataFrame({
        "Pitcher": ["Pitcher 1", "Pitcher 2"],
        "Avg Velocity": [
            float(p1["release_speed"].mean()) if "release_speed" in p1 and not p1.empty else 0.0,
            float(p2["release_speed"].mean()) if "release_speed" in p2 and not p2.empty else 0.0
        ],
        "Avg Spin": [
            float(p1["release_spin_rate"].mean()) if "release_spin_rate" in p1 and not p1.empty else 0.0,
            float(p2["release_spin_rate"].mean()) if "release_spin_rate" in p2 and not p2.empty else 0.0
        ]
    })
    st.bar_chart(df.set_index("Pitcher"))


def render_pitch_movement_scatter(p1, p2):
    st.subheader("Pitch Movement Scatter")
    frames = []
    for data, label in [(p1, "Pitcher 1"), (p2, "Pitcher 2")]:
        if not data.empty and {'pfx_x', 'pfx_z'}.issubset(data.columns):
            frame = data[['pfx_x', 'pfx_z']].dropna().copy()
            frame['Pitcher'] = label
            frames.append(frame)
    if not frames:
    ...
    fig = px.scatter(
        ...
        color="Pitcher",
        opacity=0.6,
        title="Pitch Break Movement (pfx_x / pfx_z)",
...
def render_release_point_cluster(p1, p2):
    st.subheader("Release Point Cluster")
    if p1.empty and p2.empty:
        st.info("No data available for comparison.")
        return
    fig = px.scatter(
        pd.concat([
            p1.assign(Pitcher="Pitcher 1") if not p1.empty else pd.DataFrame(),
            p2.assign(Pitcher="Pitcher 2") if not p2.empty else pd.DataFrame()
        ]),
        x="release_speed",
        y="z_norm",
        color="Pitcher",
        opacity=0.6,
        title="Release Point Cluster (Speed vs Normalized Height)",
        template="plotly_dark"
    )
    st.plotly_chart(fig, use_container_width=True)


def render_whiff_chase_comparison(p1, p2):
    st.subheader("Whiff & Chase Rate Comparison")

    def calc_rates(df):
        if df.empty:
            return 0.0, 0.0
        whiff = int(df["is_whiff"].sum()) if "is_whiff" in df else 0
        swings = int(df["is_swing"].sum()) if "is_swing" in df else 0
        chase = int((df["is_swing"] & ~df["is_in_zone"]).sum()) if "is_swing" in df and "is_in_zone" in df else 0
        chase_opp = int((~df["is_in_zone"]).sum()) if "is_in_zone" in df else 0
        return (
            (whiff / swings) if swings else 0.0,
            (chase / chase_opp) if chase_opp else 0.0
        )

    p1_whiff, p1_chase = calc_rates(p1)
    p2_whiff, p2_chase = calc_rates(p2)

    df = pd.DataFrame({
        "Pitcher": ["Pitcher 1", "Pitcher 2"],
        "Whiff Rate": [p1_whiff, p2_whiff],
        "Chase Rate": [p1_chase, p2_chase]
    })
    st.bar_chart(df.set_index("Pitcher"))


def render_zone_profile_overlay(p1, p2):
    st.subheader("Zone Profile Overlay")
    fig = go.Figure()
    if not p1.empty:
        fig.add_trace(go.Scatter(
            x=p1["plate_x"], y=p1["plate_z"],
            mode="markers", name="Pitcher 1", opacity=0.5
        ))
    if not p2.empty:
        fig.add_trace(go.Scatter(
            x=p2["plate_x"], y=p2["plate_z"],
            mode="markers", name="Pitcher 2", opacity=0.5
        ))
    fig.update_layout(title="Zone Profile Overlay", template="plotly_dark")
    st.plotly_chart(fig, use_container_width=True)


def render_pitch_mix_distribution(p1, p2):
    st.subheader("Pitch Mix Distribution")

    def mix(df):
        if df.empty or "pitch_type" not in df:
            return pd.Series(dtype=float)
        return df["pitch_type"].value_counts(normalize=True)
...
def render_hot_cold_zone_comparison(p1, p2):
    st.subheader("Hot/Cold Zone Comparison")

    def zone(df):
        if df.empty or "plate_z" not in df or "is_whiff" not in df:
            return pd.Series(dtype=float)
        return df.groupby(pd.cut(df["plate_z"], bins=5))["is_whiff"].mean()

    z1 = zone(p1)
    z2 = zone(p2)
    df = pd.DataFrame({"Pitcher 1": z1, "Pitcher 2": z2}).fillna(0)
    st.line_chart(df)


def render_run_value_by_pitch_type(p1, p2):
    st.subheader("Run Value by Pitch Type")

    def rv(df):
        if df.empty or "pitch_type" not in df or "delta_run_exp" not in df:
            return pd.Series(dtype=float)
        return df.groupby("pitch_type")["delta_run_exp"].mean()
...
def render_expected_vs_actual(p1, p2):
    st.subheader("Expected vs Actual Outcomes")

    def outcomes(df):
        if df.empty:
            return pd.DataFrame({"Expected": [0.0], "Actual": [0.0]})
        return pd.DataFrame({
            "Expected": [float(df["release_speed"].mean()) if "release_speed" in df else 0.0],
            "Actual": [float(df["post_home_score"].mean()) if "post_home_score" in df else 0.0]
        })

    df = pd.concat([
        outcomes(p1).assign(Pitcher="Pitcher 1"),
        outcomes(p2).assign(Pitcher="Pitcher 2")
    ])
    st.bar_chart(df.set_index("Pitcher"))


# -----------------------------------------------------------------------------
# 11. PAGE 6: ⚔️ PLAYER COMPARISON
# -----------------------------------------------------------------------------
if app_mode == "⚔️ Player Comparison":
    st.title("⚔️ Player Comparison")
    st.caption("Compare two pitchers across velocity, movement, pitch mix, and command outcomes.")

    if len(pitcher_list) < 2:
        st.info("At least two pitchers are required. Lower the minimum-pitch filter or select another season.")
    else:
        selected_comparison = st.sidebar.multiselect(
            "Select exactly two pitchers:",
            options=pitcher_list,
            default=pitcher_list[:2],
            max_selections=2,
            key="comparison_pitchers"
        )

        if len(selected_comparison) != 2:
            st.info("Select two pitchers in the sidebar to start the comparison.")
        else:
            first_name, second_name = selected_comparison
            with st.spinner("Loading comparison data..."):
                first_data, _ = fetch_pitcher_statcast(first_name, selected_season)
                second_data, _ = fetch_pitcher_statcast(second_name, selected_season)

            st.subheader(f"{first_name} vs {second_name}")
            if first_data.empty and second_data.empty:
                st.warning("Statcast data is unavailable for both selected pitchers.")
            else:
                render_velocity_spin_comparison(first_data, second_data)
                render_whiff_chase_comparison(first_data, second_data)
                render_pitch_mix_distribution(first_data, second_data)
                render_pitch_movement_scatter(first_data, second_data)
                render_release_point_cluster(first_data, second_data)
                render_zone_profile_overlay(first_data, second_data)


# -----------------------------------------------------------------------------
# 12. PAGE 7: 🧑‍🤝‍TEAM PITCHERS
# -----------------------------------------------------------------------------
def render_team_pitchers_page():
    ...
    if not team_pitchers_df.empty:
        ...
        st.info(f"No active pitching statistics were found for {selected_team_full} in {selected_season}.")
    
if app_mode == "🧑‍🤝‍Team Pitchers":
    render_team_pitchers_page()

# =============================================================================
# FOOTER & FINAL ENHANCEMENTS
# =============================================================================
...
st.markdown(
    ...
    <div style='text-align: center; color: #888; font-size: 0.85rem; margin-top: 20px;'>
        ⚾ <b>MLB Pitcher Analytics Hub</b><br>
        Powered by Statcast & Baseball Savant APIs | Real-time data updated daily<br>
        <small>Built with Streamlit, Pandas, PyBaseball, and Plotly</small>
    </div>
    """,
    unsafe_allow_html=True
)
